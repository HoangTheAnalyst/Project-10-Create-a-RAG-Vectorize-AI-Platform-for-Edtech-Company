...
## 7.Add Users & Permissions from Snowflake (example:STREAMLIT user)
```sql
-- Switch to administrator role
USE ROLE ACCOUNTADMIN;

-- 1. CREATE ROLE FOR STREAMLIT (IF NOT EXISTS)
CREATE ROLE IF NOT EXISTS STREAMLIT_ROLE;

-- 2. CREATE USER AND ASSIGN DEFAULT ROLE/WAREHOUSE
CREATE USER IF NOT EXISTS STREAMLIT
    PASSWORD = 'streamlit@'
    DEFAULT_WAREHOUSE = 'COMPUTE_WH'
    MUST_CHANGE_PASSWORD = FALSE;

GRANT ROLE STREAMLIT_ROLE TO USER STREAMLIT;
-- 3. GRANT USAGE PRIVILEGES ON WAREHOUSE & DATABASE
GRANT USAGE ON WAREHOUSE COMPUTE_WH TO ROLE STREAMLIT_ROLE;
GRANT USAGE ON DATABASE RAG_AI_PLATFORM TO ROLE STREAMLIT_ROLE;

-- 4. RAW LAYER PRIVILEGES (FOR STREAMLIT LOGGING & RAW INSPECTION)

GRANT USAGE ON SCHEMA RAG_AI_PLATFORM.RAW TO ROLE STREAMLIT_ROLE;
GRANT USAGE ON SCHEMA RAG_AI_PLATFORM.MARTS_LOG TO ROLE STREAMLIT_ROLE;
-- Allow Streamlit to both INSERT new query logs and SELECT raw conversation history
GRANT SELECT, INSERT ON TABLE RAG_AI_PLATFORM.RAW.QUERY_LOGS TO ROLE STREAMLIT_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA RAG_AI_PLATFORM.RAW TO ROLE STREAMLIT_ROLE;

GRANT SELECT ON TABLE RAG_AI_PLATFORM.MARTS_LOG.FCT_LOG TO ROLE STREAMLIT_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA RAG_AI_PLATFORM.MARTS_LOG TO ROLE STREAMLIT_ROLE;

-- 5. MARTS LAYER PRIVILEGES (FOR VECTOR RETRIEVAL / KNOWLEDGE BASE)

GRANT USAGE ON SCHEMA RAG_AI_PLATFORM.MARTS TO ROLE STREAMLIT_ROLE;

-- Allow Streamlit to query knowledge base and metadata
GRANT SELECT ON TABLE RAG_AI_PLATFORM.MARTS.FCT_CHUNKS TO ROLE STREAMLIT_ROLE;
GRANT SELECT ON TABLE RAG_AI_PLATFORM.MARTS.DIM_LESSON TO ROLE STREAMLIT_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA RAG_AI_PLATFORM.MARTS TO ROLE STREAMLIT_ROLE;
```
...
renamed AS (
    SELECT
        ...
        -- If any selected_subject or selected_lesson is 'Tất cả' (Old Version), replace it with 'All' (New Version) for consistency
        CASE
            WHEN TRIM(selected_subject) = 'Tất cả' THEN 'All'
            ELSE CAST(selected_subject AS VARCHAR)
        END AS selected_subject,
        CASE
            WHEN TRIM(selected_lesson) = 'Tất cả' THEN 'All'
            ELSE CAST(selected_lesson AS VARCHAR)
        END AS selected_lesson,

...
# ----------------------------------------------------------------------
def chunk_lecture_markdown(
    md_content: str,
    subject: str,
    lesson_name: str,
    file_name: str,
) -> list:
    """Splits a lecture Markdown document into structured chunks based on '### ' headers."""
    lines = md_content.split("\n")
    chunks = []

    current_lines = []
    current_section = lesson_name
    chunk_index = 1
    ...
    def flush_chunk():
        ...
        if body:
            chunk_payload = {
                "chunk_id": f"{sanitized_lesson}_th_{chunk_index:04d}",
                "file_name": file_name,
                "subject": subject,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "CLI Tool to parse and split Markdown lecture documents into"
    ...
    parser.add_argument(
        ...
        "--output",
        type=str,
        default="documents/chunking_documents/Lecture",
        help=(
            "Path to destination JSON directory (Default:"
            " documents/chunking_documents/Lecture)"
        ),
    )
